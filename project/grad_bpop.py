...
def helper_v_EBE (model_switch,xt_ind,x,a,bpop,b_ind,bocc_ind,d,sigma,docc,poped_db):
  ...
  return(list(vx=vx, poped_db=poped_db))
}

# helper for m1
def helper_LinMatrix (model_switch,xt_ind,x,a,bpop,b_ind,bocc_ind,d,sigma,docc,poped_db):
  
  epsi0 = zeros(1,len(poped-db$parameters$notfixed_sigma))
  
  # create linearized model
  if((poped_db$settings$iApproximationMethod==0 || poped_db$settings$iApproximationMethod==3) ){#FO, FOI
  ...
  if ( (poped_db$settings$iApproximationMethod==0 || poped_db$settings$iApproximationMethod==3 || (isempty(b_ind) && isempty(bocc_ind))) ) {
    #FO, FOI
  ...
  return(list(ferror = ferror, poped_db = poped_db))
