...
from project.models import ff_PK_1_comp_oral_md_KE
...
def sfg(x,a,bpop,b,bocc):
    parameters = Matrix(np.array([bpop.get_data()[0]*np.exp(b.get_data()[0]), 
                                ...
                                a.get_data()[1]]),
                        (1, 6),
                        ["V", "KA", "KE", "Favail", "DOSE", "TAU"],
                        None, None)
    return parameters

## -- Define design and design space
poped_db = create_poped_database(ff_fun=ff_PK_1_comp_oral_md_KE,
                                  fg_fun=sfg,
                                  fError_fun=feps_add_prop,
                                  groupsize=20,
                                  m=2,
                                  sigma=Matrix(np.array([0.04,5e-6])),
                                  bpop=Matrix(np.array([72.8,0.25,3.75/72.8,0.9])), 
