...
def graddetmf(model_switch,aX,groupsize,ni,xt,x,a,bpop,d,sigma,docc,poped_db,lndet=False,gradxt=False):
    ...
    for(p in 1:iParallelN){
        ...
        if(groupsize[i]==0){
            ...
            if((iParallelN ==1)){
            returnArgs <- mf_all(t(model_switch[i,1:ni[i,drop=F],drop=F]),t(xt[i,1:ni[i,drop=F],drop=F]),x_i,a_i,bpop,d,sigma,docc,poped_db) 
            mf_tmp <- returnArgs[[1]]
            poped_db <- returnArgs[[2]]
            } else {
            if((p==1)){
        ...
        a0 = a
        xt0 = xt
        nCtl = ifelse(gradxt==FALSE, size(poped_db$design$a,2), ni[i])
            for(ct1 in 1:nCtl){
            if(aX[i,ct1]!=0){
            if (gradxt==FALSE) {
                a=a0
                ...
                if((iParallelN ==1)){
                returnArgs <- mf_all(t(model_switch[i,1:ni[i,drop=F],drop=F]),t(xt[i,1:ni[i,drop=F],drop=F]),x_i,t(a[i,,drop=F]),bpop,d,sigma,docc,poped_db) 
                mf_tmp <- returnArgs[[1]]
                poped_db <- returnArgs[[2]]
                } else {
                if((p==1)){
    ...
    return(list(ret = ret, poped_db = poped_db)) 
}
